...
class TransactionServiceImplTest {
    ...
    @Test
    void findTransactionsByBookingId_shouldMapAndReturnDtoList() {
        Long bookingId = 1L;
        Transaction transaction = Transaction.builder().booking(Booking.builder().id(bookingId).build()).build();
        ...
        List<TransactionDto> results = transactionService.findTransactionsByBookingId(bookingId);

        assertEquals(1, results.size());
        assertEquals(bookingId, results.get(0).getBookingId());
    }
}