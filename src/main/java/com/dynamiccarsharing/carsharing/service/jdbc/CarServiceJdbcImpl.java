...
public class CarServiceJdbcImpl implements CarService {
    ...
    public CarServiceJdbcImpl(CarRepositoryJdbcImpl carRepositoryJdbcImpl) {
        this.carRepositoryJdbcImpl = carRepositoryJdbcImpl;
    }

    @Override
    public Car save(Car car) {
        return carRepositoryJdbcImpl.save(car);
    }

    @Override
    @Transactional(readOnly = true)
    public Optional<Car> findById(Long id) {
        return carRepositoryJdbcImpl.findById(id);
    }

    @Override
    public void deleteById(Long id) {
        getCarOrThrow(id);
        carRepositoryJdbcImpl.deleteById(id);
    }

    @Override
    @Transactional(readOnly = true)
    public Iterable<Car> findAll() {
        return carRepositoryJdbcImpl.findAll();
    }
    ...
    public Car rentCar(Long carId) {
        ...
        return carRepositoryJdbcImpl.save(car.withStatus(CarStatus.RENTED));
    }
    ...
    public Car returnCar(Long carId) {
        ...
        return carRepositoryJdbcImpl.save(car.withStatus(CarStatus.AVAILABLE));
    }
    ...
    public Car setMaintenance(Long carId) {
        ...
        return carRepositoryJdbcImpl.save(car.withStatus(CarStatus.MAINTENANCE));
    }
    ...
    public Car verifyCar(Long carId) {
        ...
        return carRepositoryJdbcImpl.save(car.withVerificationStatus(VerificationStatus.VERIFIED));
    }
    ...
    public Car rejectVerification(Long carId) {
        ...
        return carRepositoryJdbcImpl.save(car.withVerificationStatus(VerificationStatus.REJECTED));
    }
    ...
    public Car updatePrice(Long carId, BigDecimal newPrice) {
        ...
        Car car = getCarOrThrow(carId);
        return carRepositoryJdbcImpl.save(car.withPrice(newPrice));
    }

    @Override
    @Transactional(readOnly = true)
    public List<Car> searchCars(CarSearchCriteria criteria) {
        Filter<Car> filter = createFilterFromCriteria(criteria);
        try {
            return carRepositoryJdbcImpl.findByFilter(filter);
        } catch (SQLException e) {
            throw new RuntimeException("Search for cars failed", e);
        }
    }

    private Filter<Car> createFilterFromCriteria(CarSearchCriteria criteria) {
        return CarFilter.of(
                criteria.getMake(),
                criteria.getModel(),
                criteria.getStatus(),
    ...
    private Car getCarOrThrow(Long carId) {
        return carRepositoryJdbcImpl.findById(carId).orElseThrow(() -> new CarNotFoundException("Car with ID " + carId + " not found"));
    }
