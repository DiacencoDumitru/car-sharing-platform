...
public class Validator {
    ...
    public static void validateOptionalString(String value, String fieldName) {
        if (value != null && value.trim().isEmpty()) {
            throw new IllegalArgumentException(fieldName + " must be non-empty if provided");
        }
    }
    ...
    public static void validateEmail(String email, String fieldName) {
        if (email == null || email.trim().isEmpty() || !email.matches("^[\\w-\\.]+@([\\w-]+\\.)+[\\w-]{2,4}$")) {
            throw new IllegalArgumentException(fieldName + " must be non-null, non-empty, and a valid email address");
        }
    }

    public static void validateDates(LocalDateTime start, LocalDateTime end, String startFieldName, String endFieldName) {
        if (start == null || end == null) {
            throw new IllegalArgumentException(startFieldName + " and " + endFieldName + " must be non-null");
        }
