...
class ContactInfoServiceImplTest {
    ...
    void deleteById_whenContactInfoExists_shouldSucceed() {
        Long contactId = 1L;
        when(contactInfoRepository.findById(contactId)).thenReturn(Optional.of(createTestContactInfo(contactId)));
    ...
    @Test
    void searchContactInfo_withCriteria_shouldCallRepositoryWithSpecification() throws SQLException {
        String firstName = "Dumitru";
        ContactInfoSearchCriteria criteria = ContactInfoSearchCriteria.builder().firstName(firstName).build();
        when(contactInfoRepository.findByFilter(any(Filter.class))).thenReturn(List.of(createTestContactInfo(1L)));

        List<ContactInfo> results = contactInfoService.searchContactInfo(criteria);

        assertFalse(results.isEmpty());
        verify(contactInfoRepository, times(1)).findByFilter(any(Filter.class));
    }
}