...
class CarServiceJpaTest {

    @Mock
    private CarJpaRepository carRepository;

    private CarServiceJpaImpl carService;

    @BeforeEach
    void setUp() {
        carService = new CarServiceJpaImpl(carRepository);
    }
    ...
    void searchCars_shouldCallRepositoryWithSpecification() {
        ...
        when(carRepository.findAll(any(Specification.class))).thenReturn(List.of(createTestCar(1L, CarStatus.AVAILABLE, VerificationStatus.VERIFIED)));
