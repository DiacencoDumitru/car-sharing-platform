...
class UserReviewServiceImplTest {
    ...
    void updateReview_whenExists_shouldSucceedAndReturnDto() {
        ...
        doNothing().when(userReviewMapper).updateFromDto(updateDto, existingReview);
        when(userReviewRepository.save(existingReview)).thenReturn(existingReview);
        when(userReviewMapper.toDto(existingReview)).thenReturn(expectedDto);

        UserReviewDto result = userReviewService.updateReview(reviewId, updateDto);

        assertNotNull(result);
        assertEquals("New Comment", result.getComment());
        verify(userReviewMapper).updateFromDto(updateDto, existingReview);
